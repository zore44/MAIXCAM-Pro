...
class MaixCamProThresholdTool:
    ...
    def _draw_button(self, img, btn_info, selected=False):
        ...
        img.draw_string(x + 2, y + 5, text, image.COLOR_WHITE, scale=0.6)
    ...
    def run(self):
        ...
        while not self.exit_flag:
            ...
            for param, value in self.hsv_params.items():
                ...
                img_show.draw_string(205, y_offset, f"{param}:{value}", color, scale=0.6)
                y_offset += 12
...
def get_hsv():
    """一句话获取HSV阈值"""
    return MaixCamProThresholdTool().run()
